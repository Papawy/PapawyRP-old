...
stock SendProxPlayerText(playerid, text[])
{
	...
	foreach(new i : Player)
	{
		...
		{
			SendClientMessageFormatted(i, PROX_TCHAT_COLOR_5, PROX_TCHAT_FORMAT, GetPlayerNameEx(playerid), text);
		}
		else if(dist <= PROX_TCHAT_RADIUS_4)
		{
			SendClientMessageFormatted(i, PROX_TCHAT_COLOR_4, PROX_TCHAT_FORMAT, GetPlayerNameEx(playerid), text);
		}
		else if(dist <= PROX_TCHAT_RADIUS_3)
		{
			SendClientMessageFormatted(i, PROX_TCHAT_COLOR_3, PROX_TCHAT_FORMAT, GetPlayerNameEx(playerid), text);
		}
		else if(dist <= PROX_TCHAT_RADIUS_2)
		{
			SendClientMessageFormatted(i, PROX_TCHAT_COLOR_2, PROX_TCHAT_FORMAT, GetPlayerNameEx(playerid), text);
		}
		else if(dist <= PROX_TCHAT_RADIUS_1)
		{
			SendClientMessageFormatted(i, PROX_TCHAT_COLOR_1, PROX_TCHAT_FORMAT, GetPlayerNameEx(playerid), text);
		}
	}
	return true;
}
...
stock SendPlayerAction(playerid, action[])
{
	...
	foreach(new i : Player)
	{
		...
		{
			SendClientMessageFormatted(i, CMD_ACTION_COLOR, "* %s %s", GetPlayerNameEx(playerid), action);
		}
	}
	return true;
}
...
stock SendPlayerDescription(playerid, description[])
{
	...
	foreach(new i : Player)
	{
		if(IsPlayerInRangeOfPoint(i, CMD_ACTION_RADIUS, pos[0], pos[1], pos[2]))
		{
			SendClientMessageFormatted(i, CMD_ACTION_COLOR, "* %s (%s)", description, GetPlayerNameEx(playerid));
		}
	}
	return true;
}